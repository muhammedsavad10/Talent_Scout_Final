...
const RecruiterDashboard = ({ activeRole = 'Recruiter' }) => {
  ...
  useEffect(() => {
    if (result) {
      ...
      setOverrideDecision(result.recommendation?.hiring_recommendation || '');
      setEmailDraft(null);
      setChatMessages([
        {
          role: 'assistant',
          content: `Hello! I have loaded the evaluation for **${result.filename}**. You can ask me questions about their technical skills, projects, or work history, and I will cite evidence directly from their resume.`,
          citations: []
        }
      ]);
    }
  ...
  const pollBatchJob = (bId) => {
    ...
    const pollInterval = setInterval(async () => {
      try {
        if (Date.now() - startTime > 30 * 60 * 1000) { // 30 mins max timeout for huge batches
        ...
        if (res.data.status === 'COMPLETED' || res.data.status === 'COMPLETED_WITH_ERRORS' || res.data.status === 'FAILED') {
          ...
          if (res.data.status === 'COMPLETED_WITH_ERRORS') {
            ...
            // Auto-load top candidate result for Dev Console
  ...
  const handleViewResult = async (candRow) => {
    ...
    try {
      ...
      if (validStates.includes(response.data.status) && response.data.result) {
        setResult(response.data.result);
        setEvaluationId(candRow.evaluation_id);
        localStorage.setItem('lastEvaluation', JSON.stringify(response.data.result));
        setActiveStep(2);
      } else {
