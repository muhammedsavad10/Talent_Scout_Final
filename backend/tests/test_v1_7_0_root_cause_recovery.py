"""
TalentScout Enterprise v1.7.0 — Root Cause Investigation & Runtime Truth Recovery Test Suite.
Validates 100% truth recovery for Devadethan, Shadin, Muhammad, and Adhil:
1. Devadethan: current_company == "Prevalent AI", current_role == "Data Scientist L1" (NOT Unknown).
2. Shadin: current_company == "Bridgeon Solutions", current_role == "Data Analyst" (NOT Unknown).
3. Muhammad: current_company == "Unknown", current_role == "Unknown", work_history == [], Delay2Decision & FairCrop AI exist ONLY under projects.
4. Adhil: certifications contain ONLY genuine credentials, zero action-verb project bullets.
5. Benchmark candidate ranking remains Devadethan > Muhammad > Shadin > Adhil.
"""
import pytest
import asyncio
...
@pytest.mark.asyncio
async def test_devadethan_runtime_truth_recovery():
    res = await run_evaluation_pipeline(DEVADETHAN_RESUME, "eval_devadethan_v17", required_skills=["Python"])
    ...
    hp_profile = final_json.get("hiring_priority", {}).get("professional_profile", {})
    assert hp_profile.get("current_company") == "Prevalent AI"
    assert hp_profile.get("current_role") == "Data Scientist L1"

@pytest.mark.asyncio
async def test_shadin_runtime_truth_recovery():
    res = await run_evaluation_pipeline(SHADIN_RESUME, "eval_shadin_v17", required_skills=["Python"])
    final_json = validate_final_api_response(res)
...
@pytest.mark.asyncio
async def test_muhammad_no_project_employment_leakage():
    res = await run_evaluation_pipeline(MUHAMMAD_RESUME, "eval_muhammad_v17", required_skills=["Python"])
    final_json = validate_final_api_response(res)
    
    assert final_json.get("current_company") == "Unknown"
    assert final_json.get("current_role") == "Unknown"
    
    inner_eval = final_json.get("evaluation", {})
    assert inner_eval.get("work_history") == []
    
    project_titles = [p.get("title") for p in inner_eval.get("parsed_resume", {}).get("projects", [])]
    assert "Delay2Decision" in project_titles or "FairCrop AI" in project_titles

@pytest.mark.asyncio
async def test_adhil_no_action_verb_certifications():
    res = await run_evaluation_pipeline(ADHIL_RESUME, "eval_adhil_v17", required_skills=["Python"])
    final_json = validate_final_api_response(res)
...
def test_v1_7_0_benchmark_candidate_ranking_preservation():
    cand_devadethan = {
        "overall_score": 96.0,
